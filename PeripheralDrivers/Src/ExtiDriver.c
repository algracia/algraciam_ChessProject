...
void ExtInt_Config(EXTI_Config_t *extiConfig){
	...
	case 1: {
		...
		//Ahora escribimos sobre el registro, segun el GPIOx que estemos usando
        if(extiConfig->pGPIOHandler->pGPIOx == GPIOA){
	...
	if(extiConfig->edgeType == EXTERNAL_INTERRUPT_FALLING_EDGE){
		...
		EXTI->RTSR &= ~(SET << extiConfig->pGPIOHandler->GPIO_PinConfig.GPIO_PinNumber); 	//deshabilitando cualquier flanco de subida para esos pines
        EXTI->FTSR |= (SET << extiConfig->pGPIOHandler->GPIO_PinConfig.GPIO_PinNumber);		//habilitando flanco de bajada
	}
	else{
		/* Rising Trigger selection register*/
		EXTI->FTSR &= ~(SET << extiConfig->pGPIOHandler->GPIO_PinConfig.GPIO_PinNumber); 	//deshabilitando cualquier flanco de bajada para esos pines
        EXTI->RTSR |= (SET << extiConfig->pGPIOHandler->GPIO_PinConfig.GPIO_PinNumber);		//habilitando flanco de subida
...
}// Fin de la funcion extInt_Config
